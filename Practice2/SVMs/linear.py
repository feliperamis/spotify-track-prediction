...
cvacc = cross_val_score(SVC(kernel="linear", C=parval['C'], decision_function_shape=parval["decision_function_shape"]), X=x_train, y=y_train, cv=K, scoring="accuracy")
# Train
SVM = SVC(kernel="linear", C=parval['C'], decision_function_shape=parval["decision_function_shape"])
SVM.fit(x_train, y_train)
# Evaluate and Save
output = evaluate_classifier(start, SVM, x_test, y_test, parval, cvacc)
save_output("outputs/linear_ovo.txt", output)
